...
exports.DataCleaningController = void 0;
const dataCleaningService_1 = require("../../services/admin/dataCleaningService");
class DataCleaningController {
    dataCleaningService;
    constructor(pool) {
        this.dataCleaningService = new dataCleaningService_1.DataCleaningService(pool);
    }
    /**
     * Create a new data cleaning job
     */
    createCleaningJob = async (request, reply) => {
        try {
            const { jobId, cleaningType, config } = request.body;
            ...
            const cleaningJob = await this.dataCleaningService.createCleaningJob(finalJobId, cleaningType, config, userId);
     ...
     */
    startCleaningJob = async (request, reply) => {
        try {
            ...
            switch (cleaningJob.cleaning_type) {
                case 'duplicate_detection':
                    await this.dataCleaningService.detectDuplicates(jobId, cleaningJob.config);
                    break;
                case 'geometry_validation':
                    await this.dataCleaningService.validateGeometry(jobId, cleaningJob.config);
                    break;
                case 'attribute_standardization':
                    await this.dataCleaningService.standardizeAttributes(jobId, cleaningJob.config);
                    break;
                default:
                    reply.status(400).send({
                        error: 'Bad Request',
     ...
     */
    getCleaningJob = async (request, reply) => {
        try {
            const { jobId } = request.params;
            const cleaningJob = await this.dataCleaningService.getCleaningJob(jobId);
            if (!cleaningJob) {
     ...
     */
    getCleaningJobs = async (request, reply) => {
        try {
            const { limit = 50, offset = 0 } = request.query;
            const jobs = await this.dataCleaningService.getCleaningJobs(parseInt(limit), parseInt(offset));
            reply.send({
                data: jobs,
                pagination: {
     ...
     */
    getCleaningIssues = async (request, reply) => {
        try {
            const { jobId } = request.params;
            const cleaningJob = await this.dataCleaningService.getCleaningJob(jobId);
            if (!cleaningJob) {
     ...
     */
    updateCleaningIssueStatus = async (request, reply) => {
        try {
            ...
            const updatedIssue = await this.dataCleaningService.updateCleaningIssueStatus(parseInt(issueId), status);
            reply.send({
                data: updatedIssue,
     ...
     */
    getCleaningStatistics = async (request, reply) => {
        try {
     ...
     */
    deleteCleaningJob = async (request, reply) => {
        try {
            const { jobId } = request.params;
            const cleaningJob = await this.dataCleaningService.getCleaningJob(jobId);
            if (!cleaningJob) {
            ...
            await this.dataCleaningService['pool'].query('DELETE FROM data_cleaning_jobs WHERE id = $1', [jobId]);
            reply.send({
...
exports.DataCleaningController = DataCleaningController;
//# sourceMappingURL=dataCleaningController.js.map