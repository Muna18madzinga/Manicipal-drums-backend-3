...
class ApprovalWorkflowService extends baseService_1.BaseService {
    constructor(pool) {
     ...
     */
    async createWorkflow(name, description, workflowType, steps, createdBy) {
        try {
            const query = `
     ...
     */
    async createApprovalRequest(workflowId, requestType, entityType, entityId, title, description, requestData, requestedBy) {
     ...
     */
    async addApprovalAction(requestId, stepNumber, actionType, actionBy, comments, actionData) {
        const query = `
    ...
    async getWorkflow(workflowId) {
        const result = await this.pool.query('SELECT * FROM approval_workflows WHERE id = $1', [workflowId]);
        return result.rows[0] || null;
    }
    /**
     * Get all workflows
     */
    async getWorkflows(workflowType) {
     ...
     */
    async getApprovalRequests(status, workflowType, limit = 50, offset = 0) {
        let query = `
     ...
     * Update request status based on action
     */
    async updateRequestStatus(requestId, actionType, stepNumber) {
    ...
    async deleteWorkflow(workflowId) {
        await this.pool.query('UPDATE approval_workflows SET is_active = false WHERE id = $1', [workflowId]);
    }
    /**
    ...
    `;
        ...
        await this.addApprovalAction(requestId, 0, // Step 0 for cancellation
        'cancel', cancelledBy, 'Request cancelled');
    }
    /**
     ...
     * Clone workflow
     */
    async cloneWorkflow(workflowId, newName, createdBy) {
        const originalWorkflow = await this.getWorkflow(workflowId);
        if (!originalWorkflow) {
            throw new Error('Original workflow not found');
        }
        const clonedWorkflow = await this.createWorkflow(newName, `${originalWorkflow.description || ''} (Cloned)`, originalWorkflow.workflow_type, originalWorkflow.steps, createdBy);
