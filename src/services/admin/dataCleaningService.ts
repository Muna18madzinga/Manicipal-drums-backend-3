...
export class DataCleaningService extends BaseService {
   ...
   */
  async generateJobId(): Promise<number> {
    try {
      const result = await this.safeQuery(
        'SELECT COALESCE(MAX(job_id), 0) + 1 as next_job_id FROM data_cleaning_jobs'
      );
   ...
   */
  async createCleaningJob(
    jobId: number,
    cleaningType: string,
    config: CleaningConfig,
    createdBy: number
  ): Promise<DataCleaningJob> {
    try {
      const query = `
   ...
   */
  async startCleaningJob(jobId: number): Promise<void> {
    try {
      await this.safeQuery(
        'UPDATE data_cleaning_jobs SET status = $1, started_at = NOW() WHERE id = $2',
        ['running', jobId]
      );
    } catch (error) {
   ...
   */
  async failCleaningJob(jobId: number, errors: any[]): Promise<void> {
    await this.pool.query(
      'UPDATE data_cleaning_jobs SET status = $1, errors = $2, completed_at = NOW() WHERE id = $3',
      ['failed', errors, jobId]
    );
  }
   ...
   * Add cleaning issue
   */
  async addCleaningIssue(
    jobId: number,
    issue: Omit<CleaningIssue, 'id' | 'created_at' | 'job_id'>
  ): Promise<CleaningIssue> {
    const query = `
   ...
   */
  async getCleaningJob(jobId: number): Promise<DataCleaningJob | null> {
    const result = await this.pool.query(
      'SELECT * FROM data_cleaning_jobs WHERE id = $1',
      [jobId]
    );
   ...
   */
  async getCleaningIssues(jobId: number): Promise<CleaningIssue[]> {
    const result = await this.pool.query(
      'SELECT * FROM cleaning_issues WHERE job_id = $1 ORDER BY severity DESC, created_at ASC',
      [jobId]
    );
   ...
   */
  async detectDuplicates(jobId: number, config: CleaningConfig): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < features.length; i++) {
        if (processed.has(features[i].id)) continue;
        
        const feature1 = features[i];
        const geom1 = JSON.parse(feature1.geom_json);
        
        for (let j = i + 1; j < features.length; j++) {
          if (processed.has(features[j].id)) continue;
   ...
   * Geometry Validation
   */
  async validateGeometry(jobId: number, config: CleaningConfig): Promise<void> {
   ...
   */
  async standardizeAttributes(jobId: number, config: CleaningConfig): Promise<void> {
   ...
   */
  private async checkGeometricDuplicate(
    geom1: any, 
    geom2: any, 
    tolerance: number
  ): Promise<boolean> {
    ...
    if (geom1.type === 'Point') {
      const distance = Math.sqrt(
        Math.pow(geom1.coordinates[0] - geom2.coordinates[0], 2) +
   ...
   */
  private async checkAttributeSimilarity(
    props1: any, 
    props2: any, 
    attributes: string[]
  ): Promise<boolean> {
   ...
   */
  private validateAttributeFormat(value: any, format: string): boolean {
    switch (format) {
      case 'email':
   ...
   */
  async getCleaningJobs(limit = 50, offset = 0): Promise<DataCleaningJob[]> {
    const query = `
   ...
   */
  async updateCleaningIssueStatus(
    issueId: number, 
    status: string
  ): Promise<CleaningIssue> {
    const query = `
