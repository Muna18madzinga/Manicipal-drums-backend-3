...
export async function spatialDataRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/test-connection', {
    ...
    try {
      ...
      return reply.code(500).send({ error: 'Database connection failed', details: (error as Error).message })
    }
  })
  ...
  fastify.get('/land-parcels', {
    ...
    try {
      const { project_id = 'default-project' } = request.query as { project_id?: string }
  ...
  fastify.get('/proposed-peri-urban-zones', {
    ...
    try {
      ...
      fastify.log.error(error)
      return reply.code(500).send({ error: 'Failed to load peri-urban zones', details: (error as Error).message })
    }
  })
    ...
    try {
      ...
      for (const layer of layers) {
        try {
          ...
          console.warn(`Could not get count for ${layer.table}:`, (error as Error).message)
        }
      }
