...
export async function spatialRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/query', {
    ...
    try {
      const { bbox, layerIds, geometryType = 'all', limit = 1000 } = request.body as any
      ...
      const params: any[] = [minX, minY, maxX, maxY]
      ...
      if (geometryType !== 'all') {
        ...
        query += ` AND ${geometryTypeMap[geometryType as keyof typeof geometryTypeMap]}`
      }
  ...
  fastify.post('/point-in-polygon', {
    ...
    try {
      const { point, layerIds } = request.body as any
      const [x, y] = point
  ...
  fastify.post('/buffer', {
    ...
    try {
      const { layerId, distance, units = 'meters', where } = request.body as any
      ...
      let query = `
        ...
        WHERE layer_id = $1
      `
      const params: any[] = [layerId, distance]
  ...
  fastify.post('/intersection', {
    ...
    try {
      const { layer1, layer2, intersectionType = 'intersects' } = request.body as any
      ...
      const { rows } = await fastify.pg.query(`
        ...
        WHERE l1.layer_id = $1 
        AND l2.layer_id = $2
        AND ${intersectionMap[intersectionType as keyof typeof intersectionMap]}
      `, [layer1, layer2])
  ...
  fastify.post('/aggregate', {
    ...
    try {
      const { layerId, aggregation } = request.body as any
      ...
      const params: any[] = [layerId, groupBy]
  ...
  // Get land parcels for a project (or all parcels if no project_id)
  fastify.get('/land-parcels', {
    schema: {
      description: 'Get land parcels for a project or all parcels',
      tags: ['Spatial'],
      querystring: {
        type: 'object',
        properties: {
        ...
        required: [] // Make project_id optional
      }
    }
  }, async (request, reply) => {
    try {
      ...
      // Add WHERE clause only if project_id is provided
      if (project_id) {
        query += ` WHERE project_id = $1`
        params.push(project_id)
      }
      
      query += ` ORDER BY stand`
  ...
  // Get surrounding parcels for site plan generation
  fastify.get('/surrounding-parcels', {
    schema: {
      description: 'Get contiguous surrounding parcels for site plan generation',
      tags: ['Spatial'],
      querystring: {
        type: 'object',
        properties: {
          selected_parcel_id: { type: 'string' }
        },
        required: ['selected_parcel_id']
      }
    }
  }, async (request, reply) => {
    try {
      const { selected_parcel_id } = request.query as { selected_parcel_id: string }
      
      // First get the selected parcel's geometry
      const selectedQuery = `
      ...
      const { rows: selectedRows } = await fastify.pg.query(selectedQuery, [selected_parcel_id])
      if (selectedRows.length === 0) {
        return reply.code(404).send({ error: 'Selected parcel not found' })
      }
      
      const selectedGeom = JSON.parse(selectedRows[0].geom)
      
      // Query for contiguous parcels (parcels that touch the selected parcel)
      const contiguousQuery = `
        SELECT 
          id, stand, description, area_m2,
          ST_AsGeoJSON(geom) as geom
        FROM land_parcels lp
        WHERE id != $1
        AND ST_Touches(lp.geom, ST_GeomFromGeoJSON($2))
        ORDER BY ST_Area(lp.geom) DESC
        LIMIT 20
      `
      
      const { rows: parcelsRows } = await fastify.pg.query(contiguousQuery, [
        selected_parcel_id,
        JSON.stringify(selectedGeom)
      ])
      
      // Format results
      const parcels = parcelsRows.map(row => ({
        type: 'Feature',
        geometry: JSON.parse(row.geom),
        properties: {
          id: row.id,
      ...
      return {
        ...
        features: parcels,
        selected_parcel: {
          id: selected_parcel_id,
          geometry: selectedGeom
        },
        count: parcels.length,
        description: 'Contiguous parcels that touch the selected parcel boundary'
      }
      
    } catch (error) {
      fastify.log.error(error)
      return reply.code(500).send({ error: 'Failed to fetch contiguous surrounding parcels' })
    }
  })
