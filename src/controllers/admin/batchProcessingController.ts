...
import { BatchProcessingService } from '../../services/admin/batchProcessingService';
import { Pool } from 'pg';
...
export class BatchProcessingController {
  ...
  constructor(pool: Pool) {
    this.batchProcessingService = new BatchProcessingService(pool);
  }

  /**
   * Create a new batch job
   */
  createBatchJob = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { name, description, job_type, config, priority, scheduled_at } = request.body as any;
      ...
      const batchJob = await this.batchProcessingService.createBatchJob(
        name,
        description || '',
        job_type,
        config || {},
   ...
   */
  addBatchJobItems = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      ...
      const batchJobItems = await this.batchProcessingService.addBatchJobItems(
        parseInt(jobId),
        items
      );
   ...
   * Get batch job by ID
   */
  getBatchJob = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { jobId } = request.params as any;
   ...
   * Get all batch jobs
   */
  getBatchJobs = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { status, job_type, limit = 50, offset = 0 } = request.query as any;
      
      const batchJobs = await this.batchProcessingService.getBatchJobs(
        status,
        job_type,
        parseInt(limit),
        parseInt(offset)
   ...
   * Start batch job
   */
  startBatchJob = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { jobId } = request.params as any;
   ...
   * Cancel batch job
   */
  cancelBatchJob = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { jobId } = request.params as any;
   ...
   * Pause batch job
   */
  pauseBatchJob = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { jobId } = request.params as any;
   ...
   * Resume batch job
   */
  resumeBatchJob = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { jobId } = request.params as any;
   ...
   */
  retryFailedItems = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { jobId } = request.params as any;
   ...
   * Get batch job items
   */
  getBatchJobItems = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { jobId } = request.params as any;
      const { status, limit = 100, offset = 0 } = request.query as any;
      
      const items = await this.batchProcessingService.getBatchJobItems(
        parseInt(jobId),
        status,
        parseInt(limit),
        parseInt(offset)
   ...
   */
  getBatchStatistics = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
   ...
   */
  createBulkImportJob = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { name, description, files, config } = request.body as any;
      const userId = (request as any).user?.id || 1;
   ...
   */
  createBulkExportJob = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { name, description, layers, format, config } = request.body as any;
      const userId = (request as any).user?.id || 1;
      ...
      // Add layer items
      const items = layers.map((layer: any) => ({
        item_type: 'record',
        item_id: layer.id,
        item_data: {
   ...
   */
  createDataCleaningJob = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { name, description, layer_id, cleaning_operations, config } = request.body as any;
      const userId = (request as any).user?.id || 1;
      ...
      const batchJob = await this.batchProcessingService.createBatchJob(
        ...
        description || '',
        'data_cleaning',
        { layer_id, cleaning_operations, ...config },
        2, // higher priority for data cleaning
        undefined, // scheduled_at
        userId
      );

      // Add cleaning operation items
      const items = cleaning_operations.map((operation: any, index: number) => ({
        item_type: 'task',
   ...
   */
  createStyleApplicationJob = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { name, description, template_id, layers, config } = request.body as any;
      const userId = (request as any).user?.id || 1;
