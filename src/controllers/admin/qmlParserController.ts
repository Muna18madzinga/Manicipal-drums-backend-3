...
import { QmlParserService } from '../../services/admin/qmlParserService';
import { Pool } from 'pg';

export class QmlParserController {
  private qmlParserService: QmlParserService;

  constructor(pool: Pool) {
    this.qmlParserService = new QmlParserService(pool);
  }

  /**
   * Create a new QML style template
   */
  createQmlTemplate = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { name, description, qml_content, style_type } = request.body as any;
      ...
      const template = await this.qmlParserService.createQmlTemplate(
        name,
        description || '',
        qml_content,
        style_type,
   ...
   */
  getQmlTemplate = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
   ...
   */
  getQmlTemplates = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { style_type } = request.query as any;
   ...
   * Update QML template
   */
  updateQmlTemplate = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { templateId } = request.params as any;
      ...
      const updatedTemplate = await this.qmlParserService.updateQmlTemplate(
        parseInt(templateId),
        updates
      );
   ...
   * Delete QML template
   */
  deleteQmlTemplate = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { templateId } = request.params as any;
   ...
   * Validate QML content
   */
  validateQmlContent = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { qml_content } = request.body as any;
   ...
   */
  parseQmlContent = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { qml_content } = request.body as any;
   ...
   */
  convertToWebStyle = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { templateId } = request.params as any;
   ...
   */
  getQmlStatistics = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      ...
      const totalSymbols = templates.reduce((sum, template) => {
        const config = template.parsed_config as any;
   ...
   * Clone QML template
   */
  cloneQmlTemplate = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { templateId } = request.params as any;
      ...
      const clonedTemplate = await this.qmlParserService.createQmlTemplate(
        name || `${originalTemplate.name} (Copy)`,
        description || `${originalTemplate.description || ''} (Cloned)`,
        originalTemplate.qml_content,
        originalTemplate.style_type,
        userId
      );
   ...
   */
  previewQmlTemplate = async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
    try {
      const { templateId } = request.params as any;
      ...
      const parsedConfig = template.parsed_config as any;
      ...
      const preview = {
        ...
        layers_count: parsedConfig.symbols?.reduce((sum: number, symbol: any) => 
          sum + (symbol.layers?.length || 0), 0) || 0,
        has_data_defined_properties: !!parsedConfig.dataDefinedProperties,
        renderer_type: parsedConfig.rendererType,
        symbols: parsedConfig.symbols?.slice(0, 3).map((symbol: any, index: number) => ({
          index: index + 1,
          type: symbol.type,
          name: symbol.name,
