...
var surveySuiteFeatures = ${JSON.stringify(Array.from(this.interactiveFeatures.entries()))};
var currentHighlight = null;
var visibleLayers = new Set(['parcels', 'beacons']);

function showFeatureAttributes(featureId) {
  const feature = surveySuiteFeatures.find(f => f[0] === featureId);
...
function searchParcel(parcelNumber) {
  const features = surveySuiteFeatures.filter(f => 
