...
export class IngestionController {
  ...
  uploadFile = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const ingestionConfig: IngestionConfig = this.parseConfig(config);
  ...
  startJob = async (req: Request, res: Response): Promise<void> => {
    try {
      if (!req.user) {
        res.status(401).json({
          error: 'Authentication required',
      ...
      const jobResult = await this.pool.query(
        'SELECT * FROM data_ingestion_jobs WHERE id = $1',
        [id]
      );
      ...
      await this.pool.query(
        'UPDATE data_ingestion_jobs SET status = $1, started_at = NOW() WHERE id = $2',
        ['processing', id]
      );
  ...
  cancelJob = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Update job status
      await this.pool.query(
        'UPDATE data_ingestion_jobs SET status = $1, completed_at = NOW() WHERE id = $2',
        ['cancelled', id]
      );
  ...
  private async createIngestionJob(jobData: Partial<IngestionJob>): Promise<IngestionJob> {
    const query = `
  ...
  private parseConfig(configString: string): IngestionConfig {
    try {
  ...
  private async validateFile(filePath: string, sourceType: string): Promise<{ valid: boolean; errors: string[] }> {
    switch (sourceType) {
      case 'shapefile':
