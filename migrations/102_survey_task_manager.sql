...
-- Survey Task Manager (formerly SurveySuite) shared objects, ported from the
-- standalone survey-suite_v1 database (pg_dump --schema-only, public -> survey).
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'survey-suite_app') THEN
    EXECUTE format('GRANT USAGE ON SCHEMA %I TO survey-suite_app', v_schema_name);
    EXECUTE format('GRANT ALL ON ALL TABLES IN SCHEMA %I TO survey-suite_app', v_schema_name);
    EXECUTE format('GRANT ALL ON ALL SEQUENCES IN SCHEMA %I TO survey-suite_app', v_schema_name);
    
    EXECUTE format('ALTER DEFAULT PRIVILEGES IN SCHEMA %I GRANT ALL ON TABLES TO survey-suite_app', v_schema_name);
    EXECUTE format('ALTER DEFAULT PRIVILEGES IN SCHEMA %I GRANT ALL ON SEQUENCES TO survey-suite_app', v_schema_name);
  END IF;
