-- Minimal Land Use Controls Migration
...
INSERT INTO land_use_groups (group_code, description, group_category, development_category, use_scale, notes, is_active, created_at)
VALUES 
    ('P', 'Prohibited Uses', 'prohibited', 'prohibited', 'all_scales', 'Uses explicitly prohibited by zoning regulations', true, NOW()),
    ('SC', 'Special Consent', 'special_consent', 'special_consent', 'all_scales', 'Uses requiring special consent/approval', true, NOW())
...
BEGIN
    ...
    IF sample_zone_id IS NOT NULL AND sample_land_use_id IS NOT NULL THEN
        INSERT INTO zone_land_use_controls (zone_id, land_use_group_id, control_type, authority, conditions)
        VALUES (sample_zone_id, sample_land_use_id, 'permitted', 'default', 'Sample permitted use relationship');
    END IF;
END $$;
