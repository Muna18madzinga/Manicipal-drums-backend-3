-- Enhanced Land Use Management Migration
-- Adds development control categories and zone-land use relationship management
-- Supports three-tier development control: Permitted, Prohibited, Special Consent
...
-- Update zones with proper types based on proposed_peri_urban_zones data
UPDATE land_zones SET 
    zone_type = CASE 
        WHEN zone_name ILIKE '%communal%' THEN 'Communal Farming Zone'
        WHEN zone_name ILIKE '%commercial%' THEN 'High Intensive Commercial Farming Zone'
        WHEN zone_name ILIKE '%estates%' THEN 'Estates Zone (Large Farms)'
        WHEN zone_name ILIKE '%irrigation%' THEN 'Irrigation Scheme Zone'
        WHEN zone_name ILIKE '%peri_urban%' THEN 'Proposed Peri-Urban Zone'
        ELSE 'Mixed Zone'
    END,
    scale_category = CASE
        WHEN zone_name ILIKE '%communal%' THEN 'small_scale'
        WHEN zone_name ILIKE '%commercial%' THEN 'large_scale'
        WHEN zone_name ILIKE '%estates%' THEN 'large_scale'
        WHEN zone_name ILIKE '%irrigation%' THEN 'mixed_scale'
        WHEN zone_name ILIKE '%peri_urban%' THEN 'mixed_scale'
        ELSE 'mixed_scale'
    END,
    zone_description = CASE
        WHEN zone_name ILIKE '%communal%' THEN 'Small-scale subsistence farming areas'
        WHEN zone_name ILIKE '%commercial%' THEN 'Large-scale commercial agriculture operations'
        WHEN zone_name ILIKE '%estates%' THEN 'Very large farms such as tea plantations'
        WHEN zone_name ILIKE '%irrigation%' THEN 'Mixed residential and agricultural areas with irrigation'
        WHEN zone_name ILIKE '%peri_urban%' THEN 'Proposed urban development zones'
...
CREATE TABLE zone_land_use_controls (
    ...
    zone_id UUID NOT NULL REFERENCES land_zones(id) ON DELETE CASCADE,
...
JOIN land_zones z ON zlc.zone_id = z.id
...
FROM land_zones z
LEFT JOIN zone_land_use_controls zlc ON z.id = zlc.zone_id
GROUP BY z.id, z.zone_name, z.zone_type, z.scale_category, z.authority;
...
BEGIN
    SELECT id INTO sample_zone_id FROM land_zones LIMIT 1;
...
COMMENT ON TABLE land_zones IS 'Enhanced zones with type, scale, and authority classification';
...
VALUES ('050_enhance_land_use_management.sql', NOW(), 'completed', 'Enhanced land use management with development control categories and dynamic zone-land use relationships');
